...
export async function startGame(
  ...
  if (data.inventory) {
    gameState.inventory = data.inventory.map((item: string | import('../types/schema.generated').InventoryItemUpdate) => {
      if (typeof item === 'string') {
        return { id: '', name: item, rarity: 'common', slot_type: '', equipped: false, is_consumable: false, is_quest_item: false, effects: [] as string[], quantity: 1 };
      }
      return {
        id: item.id || '',
        name: item.name || '?',
  ...
  // Note: connectWebSocket() was already called inside initSession/joinSession,
  // so the WS may already be connected by this point.
  if (!getSession().connected) {
    await Promise.race([
      new Promise<void>((resolve) => {
        // Re-check in case it connected during the await
        if (getSession().connected) { resolve(); return; }
        const prev = null;
