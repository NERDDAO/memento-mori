...
2. EntityResolver  (in-engine, deterministic) ──►  role→entity hints   (Milestone 2)
        │
3. kernel/comprehend  ── route to be exposed (capability EXISTS in kernel) ──►  SemanticFrame {predicate, roles, confidence}
        │
...
**`POST /v1/bonfires/{bonfire_id}/kernel/comprehend`** — **ROUTE does not yet exist in `services/graph-memory`, but the comprehension CAPABILITY exists and is exercised in the `memory_kernel` library (Milestone 2 = expose it, not build it).** The earlier "zero grep hits" finding was scoped to `services/graph-memory` (the thin HTTP wrapper) — its `kernel_service.py` / `kernel_controller.py` / `kernel_routes.py` do indeed expose only `index`, `search`, and `state`, with no comprehend route. But the underlying capability is real: `memory_kernel/src/memory_kernel/construction/fcg/bridge.py` defines `FCGBridgeClient.comprehend(grammar_ref, utterance)` and `.comprehend_scoped(grammar_ref, utterance, activation_scope)` (POSTing `/v1/comprehend` and `/v1/comprehend-scoped`), `create_fcg_bridge_app()` serves both of those routes, and the in-process path `_comprehend_loaded_grammar(grammar, utterance, grammar_ref=, activation_scope=)` drives `PyFCGBackend._comprehend_query` with no sidecar. Decisively, **`runtime/kernel.py` search ALREADY comprehends the query internally** — its `fcg_comprehend` / `comprehend_attempts` diagnostics carry `activated_cxns`, status, and attempt counts, proving comprehension-of-an-utterance is a live, exercised kernel operation today. So Milestone 2 splits into two layers: **(1) the graph-memory route** — small: wrap the existing function and map `(meanings, diagnostics.activated_cxns)` → `SemanticFrameDTO`; **(2) the game grammar** — the substantive work: author the 3 cxns as a loaded FCG image so comprehend activates `move`/`attack`/`take` and binds game roles (this is the concrete form of "kernel grammar = constraint authority"). The DTOs and adapter sketch below are the contract for layer (1). The Day-1 skeleton does not call it.
...
Graph-memory route work to ship layer (1) (Milestone 2): `KernelComprehendRequest`/`Response` DTOs in `kernel_dto.py`; `KernelService.comprehend()`; a `KernelBackendProtocol.comprehend()` extension on the kernel adapter that delegates to the existing FCG comprehension function — **local mode** → `_comprehend_loaded_grammar(grammar, utterance, grammar_ref=, activation_scope=)`, **remote mode** → `FCGBridgeClient.comprehend_scoped(...)` — with an activation scope built from `allowed_construct_ids` + `entity_hints`; the route in `kernel_routes.py` mirroring the `/index` and `/search` pattern; and frame extraction that maps PropBank-style bound slots (`V`, `ARG0`, `ARG1`, `ARGM-LOC`) to canonical role names, reading `predicate` from the `V` slot, sourcing `applied_cxn_ids` from `diagnostics.activated_cxns`. This wraps an existing capability rather than building comprehension from scratch. The engine implements its client stub against this contract; the graph-memory team owns the route. Layer (2) — authoring the game grammar so those slots actually bind `move`/`attack`/`take` — is the substantive Milestone-2 effort. **None of this is on the Day-1 critical path.**
...
| FCG comprehend route / method / DTOs (in `services/graph-memory`) | **ROUTE absent; CAPABILITY exists** — comprehension is exercised today by `runtime/kernel.py` search (`fcg_comprehend` diagnostics carry `activated_cxns`). Milestone-2 = expose a thin route over the existing function, owned by graph-memory team (prep Track C) |
| Bridge sidecar `/v1/comprehend(-scoped)` | **EXISTS in `memory_kernel`** — `FCGBridgeClient.comprehend(_scoped)` + `create_fcg_bridge_app()` serve these; in-process path is `_comprehend_loaded_grammar(...)`. graph-memory just hasn't surfaced them yet |
